...
const PROMPTS_FILE = path_1.default.join(__dirname, 'prompts.json');
// --- Encryption Helper ---
...
class FilePromptReader {
    static load() {
        try {
            if (!fs_1.default.existsSync(PROMPTS_FILE))
                return {};
            const data = fs_1.default.readFileSync(PROMPTS_FILE, 'utf-8');
...
class DataStore {
    memoryComments = {};
    memoryBusinessRequirements = {};
    memoryBusinessRequirementComments = {};
    memoryAIChat = {};
    memoryAIChatFeedback = {};
    memoryEmailLoginCodes = {};
    // --- Helper for Audit Logging ---
    async logAction(context, action, module, details, status = 'success') {
    ...
    async getUsers() {
        if (!supabase_1.isSupabaseConfigured) {
            ...
            return initialData_1.INITIAL_USERS;
        }
        try {
            const { data, error } = await supabase_1.supabase.from('users').select('*').order('created_at', { ascending: true });
            if (error)
                throw error; // Strict mode: Throw error if DB fails
            if (!data)
                return [];
            return data.map(u => ({
                id: u.key,
    ...
    async getUserByEmail(email) {
        if (!supabase_1.isSupabaseConfigured) {
            const memUser = initialData_1.INITIAL_USERS.find(u => u.email === email);
            if (memUser) {
                return {
                    id: memUser.id,
                    name: memUser.name,
        ...
        try {
            const { data, error } = await supabase_1.supabase.from('users').select('*').eq('email', email).maybeSingle();
            if (error)
                throw error;
            if (!data)
                return null;
    ...
    async getUserById(id) {
        try {
            const { data, error } = await supabase_1.supabase.from('users').select('*').eq('key', id).maybeSingle();
            if (error || !data) {
                // Fallback to memory users
                const memUser = initialData_1.INITIAL_USERS.find(u => u.id === id); // Use id (string) to match
                if (memUser) {
                    return {
                        id: memUser.id,
                        name: memUser.name,
            ...
            const memUser = initialData_1.INITIAL_USERS.find(u => u.id === id);
            if (memUser) {
                return {
                    id: memUser.id,
                    name: memUser.name,
    ...
    async addUser(user, context) {
        ...
        const { error } = await supabase_1.supabase.from('users').insert(dbUser);
        if (error)
            throw error;
        await this.logAction(context, 'Create User', 'User Management', `Created user ${user.email} (${newKey})`);
        return { ...user, id: newKey };
    }
    async updateUserProfile(id, updates, context) {
        const payload = {};
        if (updates.name)
        ...
        if (updates.phone)
            payload.phone = encrypt(updates.phone);
        if (updates.bio)
    ...
    async toggleUserStatus(id, context) {
    ...
    async resetUserPassword(id, context) {
        const defaultHash = '$2b$10$.CO1H3cduPlOskTZH3V9v.saJOlGWDRIcSiLSyxm.HhhqNRUEbOru';
        const { error } = await supabase_1.supabase.from('users').update({ password_hash: defaultHash }).eq('key', id);
        if (!error) {
            await this.logAction(context, 'Reset Password', 'User Management', `Reset password for user ${id}`);
        }
        return !error;
    }
    async changePassword(id, newHash, context) {
        const { error } = await supabase_1.supabase.from('users').update({ password_hash: newHash }).eq('key', id);
        if (!error) {
    ...
    async getNavigation() {
        if (!supabase_1.isSupabaseConfigured) {
            throw new errors_1.AppError(503, 'NAV_DB_UNAVAILABLE', '导航树数据库不可用', { hint: '当前将使用静态树' });
        }
        const { data: nodes, error } = await supabase_1.supabase
            .from('navigation_nodes')
            .select('*')
            .order('sort_order', { ascending: true });
        if (error) {
            throw new errors_1.AppError(503, 'NAV_DB_UNAVAILABLE', '导航树数据库不可用', { hint: '当前将使用静态树', supabase: error });
        }
        if (!nodes || nodes.length === 0) {
            throw new errors_1.AppError(503, 'NAV_DB_EMPTY', '导航树数据库为空', { hint: '当前将使用静态树' });
        }
        // Reconstruct Tree
        const nodeMap = new Map();
        const dbIdMap = new Map(); // int_id -> key
    ...
    async updateNavigation(tree, context) {
        if (!supabase_1.isSupabaseConfigured) {
            return;
        }
        ...
        // 1. Flatten Tree
        const flatNodes = [];
        const traverse = (nodes, parentKey) => {
        ...
        const { data: existing, error: existingErr } = await supabase_1.supabase.from('navigation_nodes').select('key');
        if (existingErr)
            throw existingErr;
        ...
        if (toDelete.length > 0) {
            const { error: delErr } = await supabase_1.supabase.from('navigation_nodes').delete().in('key', toDelete);
            if (delErr)
                throw delErr;
        }
        ...
        for (const item of flatNodes) {
            const { data, error } = await supabase_1.supabase.from('navigation_nodes').upsert({
                key: item.key,
        ...
        for (const item of flatNodes) {
            if (item.parentKey) {
                ...
                if (parentId) {
                    const { error: pErr } = await supabase_1.supabase.from('navigation_nodes')
                ...
                const { error: rErr } = await supabase_1.supabase.from('navigation_nodes')
    ...
    async forceReseedNavigation() {
        await this.updateNavigation(initialData_1.NAV_TREE);
    }
    async getGlobalStandards() {
        // Try new system_configs table first
        const { data, error } = await supabase_1.supabase.from('system_configs').select('value').eq('key', 'global_standards').single();
        if (data && data.value)
            return data.value;
        // Fallback to old navigation table (migration path)
        const { data: oldData } = await supabase_1.supabase.from('navigation').select('tree').eq('id', 'global_standards').single();
        if (oldData && oldData.tree)
            return oldData.tree.content;
        return initialData_1.MOCK_GlobalStandards;
    }
    async updateGlobalStandards(content, context) {
        // Update new system_configs table
        const { error } = await supabase_1.supabase.from('system_configs').upsert({
        ...
        if (error) {
            ...
            await supabase_1.supabase.from('navigation').upsert({
                id: 'global_standards',
                tree: { content }
            });
        }
    ...
    // --- Modules ---
    async getModule(id) {
        const { data } = await supabase_1.supabase.from('modules').select('*').eq('key', id).single();
        if (data) {
            return {
                id: data.key,
                name: data.name,
        ...
        return initialData_1.MOCK_DATABASE[id] || null;
    }
    async updateModule(id, updates, context) {
        const payload = {};
        if (updates.requirements)
            payload.requirements = updates.requirements;
        if (updates.expertRequirements)
            payload.expert_requirements = updates.expertRequirements;
        if (updates.logicRules)
            payload.logic_rules = updates.logicRules;
        if (updates.uiComponents)
            payload.ui_components = updates.uiComponents;
        if (updates.knowledge)
            payload.knowledge = updates.knowledge;
        if (updates.figmaLinks)
            payload.figma_links = updates.figmaLinks;
        if (updates.prototypeImages)
            payload.prototype_images = updates.prototypeImages;
        if (updates.versions)
            payload.versions = updates.versions;
        if (updates.comments)
            payload.comments = updates.comments;
        if (updates.owners)
            payload.owners = updates.owners;
        if (updates.timeline)
            payload.timeline = updates.timeline;
        if (updates.status)
            payload.status = updates.status;
        const { error } = await supabase_1.supabase.from('modules').update(payload).eq('key', id);
        if (error) {
    ...
    async getAuditLogsCursor(pageSize = 20, cursor) {
    ...
    async addAuditLog(log) {
        ...
        await supabase_1.supabase.from('audit_logs').insert(payload);
        return log;
    }
    ...
    async getSystemErrors() {
        try {
            const { data, error } = await supabase_1.supabase.from('system_errors').select('*').order('created_at', { ascending: false }).limit(50);
            if (error || !data) {
                return [];
            }
            return data.map(e => ({
    ...
    async addSystemError(entry) {
        try {
            ...
            const { data, error } = await supabase_1.supabase.from('system_errors').insert(payload).select().single();
            if (error) {
    ...
    async getCommentsCursor(moduleId, pageSize = 20, cursor) {
        const size = Math.max(1, Math.min(pageSize || 20, 100));
        if (!supabase_1.isSupabaseConfigured) {
    ...
    async addComment(moduleId, moduleName, content, user, context) {
        ...
        const { data, error } = await supabase_1.supabase.from('comments').insert(payload).select('*').single();
        if (error)
            throw error;
        await this.logAction(context, 'Add Comment', moduleId, `Posted comment (${content.length} chars)`);
        return {
    ...
    async getBusinessRequirementsCursor(moduleId, pageSize = 20, cursor, filters) {
        const size = Math.max(1, Math.min(pageSize || 20, 100));
    ...
    async addBusinessRequirement(moduleId, title, content, tags, priority, proposerName, user, context) {
        const now = new Date().toISOString();
        const req = {
            id: Date.now().toString(),
            moduleId,
        ...
        const { data, error } = await supabase_1.supabase.from('business_requirements').insert(payload).select('*').single();
        if (error)
            throw error;
        await this.logAction(context, 'Add Business Requirement', moduleId, `title=${title}`);
        return {
    ...
    async getBusinessRequirement(id) {
        ...
        const { data, error } = await supabase_1.supabase.from('business_requirements').select('*').eq('id', idStr).is('deleted_at', null).single();
        if (error || !data)
            return null;
        return {
            id: String(data.id),
    ...
    async getBusinessRequirementCommentsCursor(requirementId, pageSize = 30, cursor) {
    ...
    async addBusinessRequirementComment(requirementId, content, user, parentId, context) {
        const now = new Date().toISOString();
        const local = {
            id: Date.now().toString(),
        ...
        const { data, error } = await supabase_1.supabase.from('business_requirement_comments').insert(payload).select('*').single();
        if (error)
            throw error;
    ...
    async getAIChatMessagesCursor(moduleId, pageSize = 20, cursor) {
        const size = Math.max(1, Math.min(pageSize || 20, 100));
        if (!supabase_1.isSupabaseConfigured) {
    ...
    async addAIChatMessage(moduleId, agentId, promptLabel, role, content, actor, context) {
        const now = new Date().toISOString();
        const local = {
            id: Date.now().toString(),
            role,
        ...
        const { data, error } = await supabase_1.supabase.from('ai_chat_messages').insert(payload).select('*').single();
        if (error) {
            ...
            if (message.includes('agent_id') || message.includes('prompt_label')) {
                ...
                const retry = await supabase_1.supabase.from('ai_chat_messages').insert(legacyPayload).select('*').single();
                if (retry.error)
                    throw retry.error;
                await this.logAction(context, 'Add AI Chat Message', moduleId, `role=${role}, chars=${content.length}`);
                return {
    ...
    async getAIChatMessageFeedbackMap(userId, messageIds, moduleId) {
        ...
        if (!supabase_1.isSupabaseConfigured) {
            ...
            for (const id of ids) {
                ...
                if (row)
                    map[id] = { favorite: !!row.favorite, reaction: row.reaction ?? null };
            }
            return map;
        }
        ...
        for (const r of data) {
            ...
            map[mid] = { favorite: !!r.favorite, reaction: r.reaction ? String(r.reaction) : null };
        }
        return map;
    }
    async setAIChatMessageFeedback(messageId, userId, patch, context) {
        if (!userId)
        ...
        if (!supabase_1.isSupabaseConfigured) {
            ...
            if (!found)
                throw new errors_1.AppError(404, 'NOT_FOUND', 'Message not found');
            if (found.msg.role !== 'model')
                throw new errors_1.AppError(400, 'ONLY_MODEL', 'Only AI reply can be feedback');
            ...
            this.memoryAIChatFeedback[key] = next;
            ...
            return { favorite: !!next.favorite, reaction: next.reaction ?? null };
        }
        const mid = Number(idStr);
        if (Number.isNaN(mid))
            throw new errors_1.AppError(400, 'ID_INVALID', 'Invalid id');
        const { data: msgRow, error: msgErr } = await supabase_1.supabase
            .from('ai_chat_messages')
        ...
        if (msgErr || !msgRow)
            throw new errors_1.AppError(404, 'NOT_FOUND', 'Message not found');
        if (String(msgRow.role) !== 'model')
            throw new errors_1.AppError(400, 'ONLY_MODEL', 'Only AI reply can be feedback');
        const payload = {
            message_id: mid,
            module_key: msgRow.module_key,
            user_id: userId
        };
        if (typeof patch.favorite === 'boolean')
            payload.favorite = patch.favorite;
        if (patch.reaction === null || patch.reaction === 'like' || patch.reaction === 'dislike')
            payload.reaction = patch.reaction;
        const { data: upserted, error: upErr } = await supabase_1.supabase
            .from('ai_chat_message_feedback')
        ...
        if (upErr)
            throw upErr;
        await this.logAction(context, 'AI Chat Feedback', String(msgRow.module_key), `message=${idStr}, favorite=${!!upserted.favorite}, reaction=${upserted.reaction || 'null'}`);
        return { favorite: !!upserted.favorite, reaction: upserted.reaction ? String(upserted.reaction) : null };
    }
    async deleteAIChatMessagePair(messageId, context) {
        ...
        if (!supabase_1.isSupabaseConfigured) {
            ...
            if (question.role === 'model') {
                if (!isAdmin)
                    throw new errors_1.AppError(403, 'FORBIDDEN', 'Only admin can delete this message');
            ...
            if (!isAdmin) {
                if (question.role !== 'user')
                    throw new errors_1.AppError(403, 'FORBIDDEN', 'Only your questions can be deleted');
                if (String(question.actorId || '') !== String(requesterId))
                    throw new errors_1.AppError(403, 'FORBIDDEN', 'Only your questions can be deleted');
                const ageMs = now.getTime() - new Date(question.timestamp).getTime();
                if (ageMs > twelveHoursMs)
                    throw new errors_1.AppError(403, 'FORBIDDEN', 'Only messages within 12 hours can be deleted');
            }
            const all = (this.memoryAIChat[found.moduleKey] || []).filter(m => !m.deletedAt);
            const reply = all
                .filter(m => m.role === 'model' && new Date(m.timestamp).getTime() > new Date(question.timestamp).getTime())
                .sort((a, b) => new Date(a.timestamp).getTime() - new Date(b.timestamp).getTime())[0];
            const ids = [question.id, reply?.id].filter(Boolean);
        ...
        const { data: target, error: targetErr } = await supabase_1.supabase
            .from('ai_chat_messages')
            .select('*')
            .eq('id', messageId)
            .is('deleted_at', null)
        ...
        if (String(target.role) === 'model') {
            if (!isAdmin)
                throw new errors_1.AppError(403, 'FORBIDDEN', 'Only admin can delete this message');
            const { data: qBefore } = await supabase_1.supabase
                .from('ai_chat_messages')
                .select('*')
                ...
                .order('created_at', { ascending: false })
                .limit(1);
            if (qBefore && qBefore[0])
                question = qBefore[0];
        }
        if (!isAdmin) {
            if (String(question.role) !== 'user')
                throw new errors_1.AppError(403, 'FORBIDDEN', 'Only your questions can be deleted');
            if (String(question.actor_id || '') !== String(requesterId))
                throw new errors_1.AppError(403, 'FORBIDDEN', 'Only your questions can be deleted');
            const ageMs = now.getTime() - new Date(String(question.created_at)).getTime();
            if (ageMs > twelveHoursMs)
                throw new errors_1.AppError(403, 'FORBIDDEN', 'Only messages within 12 hours can be deleted');
        }
        const { data: replyRows } = await supabase_1.supabase
            .from('ai_chat_messages')
            .select('id')
        ...
        const ids = [String(question.id), replyId].filter(Boolean);
        const deletedAt = now.toISOString();
    ...
    async upsertEmailLoginCode(email, codeHash, expiresAt) {
        const emailNorm = String(email || '').trim().toLowerCase();
        if (!emailNorm)
            throw new errors_1.AppError(400, 'EMAIL_REQUIRED', 'email is required');
        if (!codeHash)
            throw new errors_1.AppError(400, 'CODE_HASH_REQUIRED', 'code hash is required');
        if (!expiresAt)
            throw new errors_1.AppError(400, 'EXPIRES_REQUIRED', 'expiresAt is required');
        if (!supabase_1.isSupabaseConfigured) {
        ...
        const { error } = await supabase_1.supabase.from('auth_email_login_codes').upsert(payload, { onConflict: 'email' });
        if (error)
            throw error;
    }
    async consumeEmailLoginCode(email, code) {
        const emailNorm = String(email || '').trim().toLowerCase();
        const codeStr = String(code || '');
        if (!emailNorm || !codeStr)
            return false;
        const nowIso = new Date().toISOString();
        const sha256 = (v) => crypto_1.default.createHash('sha256').update(v).digest('hex');
        ...
        const { data, error } = await supabase_1.supabase.from('auth_email_login_codes').select('*').eq('email', emailNorm).single();
        if (error || !data)
            return false;
        if (data.used_at)
            return false;
        ...
        if (ok)
            updated.used_at = nowIso;
        const upd = await supabase_1.supabase.from('auth_email_login_codes').update(updated).eq('email', emailNorm);
        if (upd.error)
            throw upd.error;
        return ok;
    }
    ...
    async getAgents() {
        try {
            const { data, error } = await supabase_1.supabase.from('agents').select('*');
            let agents = [];
            if (!data || data.length === 0) {
                ...
                if (error) {
                    // DB Error case
                    return initialData_1.INITIAL_AGENTS;
                }
                // Empty DB case - Try seed but inside try-catch
                const mapped = initialData_1.INITIAL_AGENTS.map(a => ({
                ...
                await supabase_1.supabase.from('agents').insert(mapped);
                // Seed Prompts from Initial Data
                for (const agent of initialData_1.INITIAL_AGENTS) {
                ...
                agents = initialData_1.INITIAL_AGENTS;
            }
            else {
                agents = data.map((a) => ({
    ...
    async addAgent(agent, context) {
        ...
        const { error } = await supabase_1.supabase.from('agents').insert(dbAgent);
        if (error)
            throw error;
        ...
        return { ...agent, id: key, status: dbAgent.status };
    }
    async updateAgent(id, updates, context) {
        const payload = {};
        if (updates.name)
            payload.name = updates.name;
        if (updates.avatar)
        ...
        if (updates.systemPrompt)
            payload.system_prompt = updates.systemPrompt;
        if (updates.pmInteractionExample)
            payload.pm_interaction_example = updates.pmInteractionExample;
        if (updates.status)
            payload.status = updates.status;
        if (updates.scope)
            payload.scope = updates.scope;
        const { error } = await supabase_1.supabase.from('agents').update(payload).eq('key', id); // Update by key
        if (error)
            throw error;
        await this.logAction(context, 'Update Agent', 'Agent Management', `Updated agent ${id}`);
        return { ...updates, id };
    }
    async deleteAgent(id, context) {
        const { error } = await supabase_1.supabase.from('agents').delete().eq('key', id); // Delete by key
        if (error)
            return false;
        // Cascade delete prompts
        await supabase_1.supabase.from('agent_prompts').delete().eq('agent_id', id);
    ...
    async addAgentPrompt(agentId, prompt, context) {
        const payload = {
            agent_id: agentId,
            label: prompt.label,
        ...
        const { data, error } = await supabase_1.supabase.from('agent_prompts').insert(payload).select().single();
        if (error) {
    ...
    async updateAgentPrompt(agentId, prompt, context) {
        const payload = {
            label: prompt.label,
            content: prompt.content,
            updated_at: new Date().toISOString()
        ...
        if (prompt.updatedBy)
            payload.updated_by = prompt.updatedBy;
        if (prompt.usageCount !== undefined)
            payload.usage_count = prompt.usageCount;
        const { data, error } = await supabase_1.supabase
            .from('agent_prompts')
            .update(payload)
    ...
    async deleteAgentPrompt(agentId, promptId, context) {
        const { error } = await supabase_1.supabase.from('agent_prompts').delete().eq('id', promptId);
        if (error)
            return false;
    ...
    memIntegrations = JSON.parse(JSON.stringify(initialData_1.INITIAL_INTEGRATIONS));
    /**
     * 获取所有集成配置
    ...
    async getIntegrations() {
        try {
            const { data, error } = await supabase_1.supabase.from('integrations').select('*').order('type');
            if (error || !data) {
     ...
     */
    async updateIntegration(key, updates, context) {
        const payload = { updated_at: new Date().toISOString() };
        if (updates.config)
            payload.config = encryptConfig(updates.config);
     ...
     * @param enabled 目标状态
     */
    async toggleIntegration(key, enabled, context) {
