...
exports.DbSchemaService = void 0;
const pg_1 = require("pg");
const promise_1 = __importDefault(require("mysql2/promise"));
class DbSchemaService {
    static async fetchSchema(config) {
        if (!config)
    ...
    static async fetchMySQLSchema(config) {
        ...
        try {
            // Tables
            const [tables] = await conn.execute(`SELECT TABLE_NAME as table_name, TABLE_COMMENT as comment 
                 FROM information_schema.TABLES 
                 WHERE TABLE_SCHEMA = ? 
            ...
            for (const t of tables) {
                ...
                // Columns
                const [cols] = await conn.execute(`SELECT COLUMN_NAME as column_name, DATA_TYPE as data_type, CHARACTER_MAXIMUM_LENGTH as char_len, 
                            IS_NULLABLE as is_nullable, COLUMN_COMMENT as comment, COLUMN_KEY as column_key
                     FROM information_schema.COLUMNS
                     WHERE TABLE_SCHEMA = ? AND TABLE_NAME = ?
                     ORDER BY ORDINAL_POSITION`, [config.database, tableName]);
                // Indexes
                const [idxs] = await conn.execute(`SELECT INDEX_NAME as index_name, GROUP_CONCAT(COLUMN_NAME ORDER BY SEQ_IN_INDEX) as columns, 
                            NON_UNIQUE as non_unique
                     FROM information_schema.STATISTICS
                     WHERE TABLE_SCHEMA = ? AND TABLE_NAME = ?
                     GROUP BY INDEX_NAME, NON_UNIQUE`, [config.database, tableName]);
                // Foreign Keys
                const [fks] = await conn.execute(`SELECT CONSTRAINT_NAME as constraint_name, COLUMN_NAME as column_name, 
    ...
    static async fetchTableData(config, tableName, page = 1, pageSize = 20) {
        ...
        if (config.type === 'mysql') {
            ...
            try {
                // choose order column
                const [cols] = await conn.execute(`SELECT COLUMN_NAME as column_name 
                     FROM information_schema.COLUMNS 
...
exports.DbSchemaService = DbSchemaService;
