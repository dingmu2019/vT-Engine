...
export const ImplementationTab: React.FC<ImplementationTabProps> = ({ data, onUpdate }) => {
  const { canEditContent } = useAuth();
  const { language } = useSettings();
  const { addToast } = useToast();

  const getTasksDoc = () => {
    const docs = (data.knowledge || []).filter(k => k.tags?.includes('tasks'));
    if (docs.length === 0) return null;
    return docs.reduce((a, b) => (new Date(a.updatedAt).getTime() >= new Date(b.updatedAt).getTime() ? a : b));
  };

  const doc = getTasksDoc();
  ...
  const t = (key: string) => {
    const dict: Record<string, { en: string; zh: string }> = {
      title: { en: 'Implementation Tasks', zh: '实施任务' },
      save: { en: 'Save', zh: '保存' },
      saved: { en: 'Saved successfully', zh: '保存成功' },
      placeholder: { en: 'Paste or edit implementation tasks markdown here...', zh: '在此处粘贴或编辑实施任务 Markdown...' },
    };
